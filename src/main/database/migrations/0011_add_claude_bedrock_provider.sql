...
CREATE TABLE `__new_agent_sessions` (
	...
	CONSTRAINT "agent_sessions_provider_check" CHECK("__new_agent_sessions"."provider" in ('codex', 'claude-code', 'claude-bedrock', 'terminal')),
	CONSTRAINT "agent_sessions_status_check" CHECK("__new_agent_sessions"."status" in ('starting', 'running', 'completed', 'failed', 'terminated')),
	CONSTRAINT "agent_sessions_surface_check" CHECK("__new_agent_sessions"."surface" in ('terminal', 'chat'))
);
--> statement-breakpoint
INSERT INTO `__new_agent_sessions`("id", "task_id", "worktree_id", "provider", "surface", "status", "command", "pid", "exit_code", "last_error", "last_event_seq", "transcript_path", "started_at", "ended_at", "created_at", "updated_at") SELECT "id", "task_id", "worktree_id", "provider", "surface", "status", "command", "pid", "exit_code", "last_error", "last_event_seq", "transcript_path", "started_at", "ended_at", "created_at", "updated_at" FROM `agent_sessions`;--> statement-breakpoint
